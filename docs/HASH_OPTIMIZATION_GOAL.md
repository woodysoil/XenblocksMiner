...
- Current dirty resume checkpoint: an uncommitted `gpu_final_hashes` experiment may be present. It moves the final Argon2 `digestLong(last block -> final hash)` work onto CUDA and copies back fixed-size final hashes. Early correctness checks preserved the golden CUDA hash and some valid samples were much faster, but repeated benchmark wrapper runs produced access-violation subprocess exits with no JSON. Treat this experiment as unstable and rejected unless a narrow resource-lifetime or synchronization fix makes focused tests, golden hashes, and repeated wrapper benchmarks pass with zero invalid subprocesses. If it cannot be stabilized quickly, revert only this uncommitted experiment and record a public-safe rejection note before continuing to the next bottleneck.
