...
- Rejected fixed-b3072 three-difficulty sequence: forcing `difficulty_sequence=1,8,64` to use `batch_size_sequence=3072,3072,3072` was tested as a way to avoid mixed batch-size setup churn, but the wrapper warm-up and all measured subprocesses exited with access violations and a direct same-command confirmation also exited with access violation before JSON. Do not use fixed b3072 for the d1/d8/d64 mixed sequence on the current implementation; keep the conservative mixed shape or redesign runner memory/batch handling first.
- Rejected fixed-b2048 three-difficulty sequence: forcing `difficulty_sequence=1,8,64` to use `batch_size_sequence=2048,2048,2048` also failed as a mixed-`m` simplification. The wrapper warm-up exited with access violation, the first measured subprocess emitted JSON but exited abnormally afterward, and the remaining measured subprocesses exited with access violations before JSON. Do not use fixed b2048 for the d1/d8/d64 mixed sequence on the current implementation.
