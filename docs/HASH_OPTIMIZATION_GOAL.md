...
- Measurement-only input residual diagnostics: benchmark `timing_analysis` now derives `input_explained_ms`, `input_residual_ms`, `input_explained_to_input`, and `input_residual_pct` from `input_ms`, `keygen_ms`, and `first_block_ms`. This helps future agents distinguish true generated-input materialization overhead from first-block work before changing the hot path. Validation passed focused benchmark/compare unit tests, and a short CUDA detailed smoke showed the fields in the JSON summary with `input_residual_pct` below 1% for that sample, supporting the current focus on first-block digest/preparation rather than generic input bookkeeping.
