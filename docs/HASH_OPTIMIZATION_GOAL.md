...
- Measurement-only update: benchmark recommendation entries now include `first_block_worker_count` and `first_block_chunk_size`, so batch-size and worker-cap scans preserve the selected first-block schedule in both selected and candidate recommendation rows.
