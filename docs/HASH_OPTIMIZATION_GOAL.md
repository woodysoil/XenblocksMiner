...
- Measurement-only update: dynamic first-block chunk scans are now useful tuning evidence. Current local evidence supports continuing b1024 chunk `32` validation and keeps b2048 dynamic chunking as unresolved because the best longer candidate was noisy.
...
- Next scheduler-specific step: run a longer b2048 confirmation for dynamic chunk `32` and `64`, and optionally rerun chunk `16`, before changing any default first-block chunk policy.
