...
- Repeated main-target-only scans on a CUDA-capable local GPU support d1/b512 and d8/b3072 as current conservative low-difficulty defaults. Treat this as local evidence only, not a universal hardware limit.
...
- Miner integration update: generated CUDA mining batches now opt into the automatic dynamic chunk policy. The backend applies the policy only to covered generated-key d1 and d8 batches with at least 1024 attempts, so unsupported difficulties and small batches keep static chunking.
...
- Auto policy extension: generated-key CUDA d1 batches with at least 1024 attempts now select chunk `16` under `first_block_dynamic_chunk_auto`, while d8 keeps chunk `32` and unsupported difficulties keep forced static behavior. Validation passed focused Hash API tests, a Release CUDA rebuild, the CUDA golden hash, a d1/d64 policy smoke, and a d8 policy smoke. The d1/d64 scan supported chunk `16`, but the rebuilt d64 auto confirmation was inconsistent, so d64 was intentionally left out of the automatic policy.
