...
- Next scheduler-specific step: run a longer same-settings confirmation for the opt-in auto policy on b1024 and b2048 before changing miner defaults. If the longer confirmation holds, consider enabling auto policy only where explicit miner configuration has not requested static or manual dynamic chunking.
