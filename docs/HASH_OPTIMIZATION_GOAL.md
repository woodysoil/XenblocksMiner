...
- Next scheduler-specific step: inspect the CUDA backend request parsing and tuning helpers, then add the smallest testable auto-policy hook for generated CUDA first-block dynamic chunks. A conservative first experiment can select chunk `32` where b1024 and b2048 evidence both support it, or chunk `16` for b2048 only if the policy is difficulty/batch-size specific. Validate with focused tests, a CUDA golden hash, and same-scenario comparisons before changing miner defaults.
