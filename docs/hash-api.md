...
For larger GPUs or deeper tuning, use repeated `--scan-difficulty`, `--scan-batch-size`, `--scan-first-block-workers`, and `--scan-first-block-dynamic-chunk-size` options to generate a custom matrix without editing the script. Add `--scan-first-block-dynamic-chunk-auto` when the scan should benchmark the backend-selected dynamic chunk policy, and add `--scan-detailed-timings` when the generated scan should include detailed CUDA setup and first-block diagnostic counters. Add `first_block_dynamic_chunk_auto=true` in a manual `--scenario` for one-off backend-selected dynamic chunk policy scenarios.
...
Current conservative defaults are `2048` attempts through difficulty `1`, `3072` attempts through difficulty `8`, and `512` attempts through difficulty `64`. Treat these as starting points for future autotuning, not universal hardware limits.
