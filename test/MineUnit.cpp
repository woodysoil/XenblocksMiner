...
{// run mine loop in fixed diff until it's break
	...
	batchSize = freeMemory / 1.01 / difficulty / 1024;
	...
	while (true) {
		...
		for (const auto& item : batchItems) {
			attempts++;
			if (item.hashed.find("XEN1") != std::string::npos) {
...
void MineUnit::stat()
{
	...
	// stream << "\033[2K\r" << "Mining: " << hashtotal << " Hashes [";
	// if (hours)
	// 	stream << std::setw(2) << std::setfill('0') << hours << ":";

	// stream << std::setw(2) << std::setfill('0') << minutes << ":"
	// 	<< std::setw(2) << std::setfill('0') << seconds;
	// stream << ", " << std::fixed << std::setprecision(2) << rate << " Hashes/s, "
	...
	statCallback({ (int)3, gpuName, memoryInGB, usedMemory/(float)gpuMemory, 0, (float)rate, "", hashtotal });
}
