...
- The first automatic dynamic chunk policy should be opt-in through `first_block_dynamic_chunk_auto`, not a miner default. Manual nonzero chunk sizes take precedence, and auto-disabled `0` remains forced static chunking.
- The opt-in auto policy now selects chunk `32` for generated-key d8 CUDA batches with at least 1024 attempts. Longer same-settings confirmation was stable and positive for b1024 and b2048. The next scheduler-specific step is miner-generated CUDA integration for covered scenarios while preserving explicit CLI static and manual dynamic chunk behavior.
